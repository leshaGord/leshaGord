...
$(document).ready(function() {
	$(window).scroll(function(event){
		var s=$(this).scrollTop();   // переменную получаем пиксили прокрученные в окнетбраузера
		var w=$(this).outerWidth();  // получаем ширину окна 
		var h=$('.content').outerHeight();// Высота контентной части
		var h_b=$('.parallax').outerHeight();//Высота верхнего блока
		var p =s/h*100;// вычесление процентов прокрутки
		var p_b =s/h_b*100;// процент прокрутки только верхней части
		var o = 1-1/100*p_b;//вычесляем будущую непрозрачность
...
$(document).ready(function(){
	$('.slider').slick({
		...
		slidesToShow:1,     	//кол-во показываемых слайдов
